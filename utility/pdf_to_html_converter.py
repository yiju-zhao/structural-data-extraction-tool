        ...
        try:
            ...
            if merged_config.get("use_llm", False):
                ...
                if not os.getenv("OPENAI_API_KEY") and not os.getenv("GOOGLE_API_KEY"):
                    self.logger.warning(
                        "No API key found for LLM usage. Disabling LLM."
                    )
    ...
    def _create_html_content(self, markdown_text: str, images: dict, title: str) -> str:
        ...
        try:
            ...
            md = markdown.Markdown(
                ...
                extension_configs={
                    "codehilite": {"use_pygments": False, "noclasses": True}
                },
            )
            ...
            self.logger.warning(
                "Markdown library not available. Using basic conversion."
            )
...
def convert_pdf_for_crawl4ai(
    pdf_path: str, output_dir: Optional[str] = None, config: Optional[dict] = None
) -> str:
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert PDF to HTML using marker library"
    )
    parser.add_argument("pdf_path", help="Path to the PDF file")
    parser.add_argument("--output-dir", help="Output directory for HTML file")
    parser.add_argument(
        "--use-llm", action="store_true", help="Use LLM for improved quality"
    )
    parser.add_argument(
        "--force-ocr", action="store_true", help="Force OCR on the document"
    )
    parser.add_argument(
        "--extract-images",
        action="store_true",
        help="Extract images from PDF (default: text only)",
    )
