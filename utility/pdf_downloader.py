...
def all_pdf_download(args):
    ...
    for counter, link in enumerate(search_res):
        ...
        if filename.lower().replace(".pdf", "").endswith("supplemental"):
            if args.print_all:
                print(
                    "[{}/{}] {} - SKIPPED (supplemental)".format(
                        counter + 1, len(search_res), filename
                    )
                )
            continue
...
if __name__ == "__main__":
    ...
    # --save-here
    parser.add_argument(
        "--save-here", dest="save_here", action="store_true", help="save files here"
    )
