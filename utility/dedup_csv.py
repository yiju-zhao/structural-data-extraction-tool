    ...
    try:
        ...
        if output_file is None:
            ...
            output_file = os.path.join(
                input_dir, f"{input_basename}_dedup_{column_name}.csv"
            )
        ...
        print(
            f"Removed {original_count - deduplicated_count} duplicates ({original_count} → {deduplicated_count} rows)"
        )
        ...
        print(
            f"Error: Column '{column_name}' not found in CSV. Available columns: {list(df.columns)}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Deduplicate CSV rows based on specified column"
    )
