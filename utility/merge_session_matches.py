...
def aggregate_matches_by_session(matched_df: pd.DataFrame) -> Dict[str, Dict[str, str]]:
    ...
    for session_title, group in grouped:
        # For each session, keep at most 3 teams.
        # Prefer ordering by highest `final_score` if available. If not, fall back to
        # `cosine_similarity` if present; otherwise preserve original order.
        top_group = group
        if "final_score" in group.columns:
            # Ensure numeric sort even if read as string
            tmp = group.copy()
            tmp["final_score"] = pd.to_numeric(tmp["final_score"], errors="coerce")
            top_group = tmp.sort_values("final_score", ascending=False, na_position="last").head(3)
        elif "cosine_similarity" in group.columns:
            tmp = group.copy()
            tmp["cosine_similarity"] = pd.to_numeric(tmp["cosine_similarity"], errors="coerce")
            top_group = tmp.sort_values("cosine_similarity", ascending=False, na_position="last").head(3)
        else:
...
def merge_data(detail_df: pd.DataFrame, matched_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for idx, row in detail_df.iterrows():
        ...
        if session_title in aggregated_matches:
            ...
            detail_df.at[idx, "recommendation_reason"] = match_info[
...
def merge_session_matches(
    matched_csv_path: str, detail_csv_path: str, output_csv_path: str
) -> None:
    """
...
def main():
    ...
    if len(sys.argv) != 4:
        print(
            "Usage: python merge_session_matches.py <matched_csv> <detail_csv> <output_csv>"
        )
        print("\nExample:")
