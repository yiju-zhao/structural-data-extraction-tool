...
def process_list_strings(df):
    ...
    for column in df_copy.columns:
        ...
        if len(sample_values) > 0:
            ...
            list_like_count = sum(
                1 for val in sample_values if list_like_pattern.match(str(val))
            )
                ...
                try:
                    ...
                    if len(df.columns) > 0 and len(df) > 0:
                        ...
                        if original_count != new_count:
                            ...
                            print(
                                f"    Removed {removed_count} duplicate rows based on column '{first_column}'"
                            )
                    ...
                    print(
                        f"✓ Converted {csv_file.name} to sheet '{sheet_name}' ({len(df)} rows)"
                    )
